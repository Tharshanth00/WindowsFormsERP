    ...
    public partial class FrmInvRep : Form
    {
        ...
        public void EPDFReport(DataGridView dgw, string filename)
        {
            ...
            if (savefiledialoge.ShowDialog() == DialogResult.OK)
            {
                using (FileStream stream = new FileStream(savefiledialoge.FileName, FileMode.Create))
                {
                    ...
                    pdfdoc.Open();
                    Paragraph para = new Paragraph("Invoice Item wise Report ");
        ...
        private void TxtSer_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.CommandText = "select  BillNumber, BillCategory, CustomerName, Description, LocationID, ManualReferenceNo, SalesCategoryKeyword, StockNumber, MarketingStaffName, CustomerAddress, PONumber" 
                    + " FROM InvoiceCompletedDetails WHERE BillNumber Like '%" + TxtSer.Text + "%' or BillCategory Like '%" + TxtSer.Text + "%' or CustomerName Like '%" + TxtSer.Text + "%' or Description Like '%" + TxtSer.Text + "%' or LocationID Like '%" + TxtSer.Text + "%' or ManualReferenceNo Like '%" + TxtSer.Text + "%' or SalesCategoryKeyword Like '%" + TxtSer.Text + "%' or StockNumber Like '%" + TxtSer.Text
        ...
        {
            EPDFReport(dataGridView1, "Invoice Item wise Report-" + DateTime.Now.ToString("dd.MM.yyyy"));
            OpenFileDialog dlg = new OpenFileDialog();
            // set file filter of dialog   
            dlg.Filter = "pdf files (*.pdf) |*.pdf;";
            dlg.FileName = ("Invoice Item wise Report-" + DateTime.Now.ToString("dd.MM.yyyy") + ".pdf");
            dlg.ShowDialog();
