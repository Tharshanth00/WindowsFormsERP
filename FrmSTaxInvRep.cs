...
namespace WindowsFormsERP
{
    public partial class FrmSTaxInvRep : Form
    {
        ...
        public void EPDFReport(DataGridView dgw, string filename)
        {
            ...
            if (savefiledialoge.ShowDialog() == DialogResult.OK)
            {
                using (FileStream stream = new FileStream(savefiledialoge.FileName, FileMode.Create))
                {
                    ...
                    pdfdoc.Open();
                    Paragraph para = new Paragraph("Suspended Tax Invoice Item wise Report ");
        ...
        {
            EPDFReport(dataGridView1, "Suspended Tax Invoice Item wise Report-" + DateTime.Now.ToString("dd.MM.yyyy"));
            OpenFileDialog dlg = new OpenFileDialog();
            dlg.Filter = "pdf files (*.pdf) |*.pdf;";
            dlg.FileName = ("Suspended Tax Invoice Item wise Report-" + DateTime.Now.ToString("dd.MM.yyyy") + ".pdf");
            dlg.ShowDialog();
